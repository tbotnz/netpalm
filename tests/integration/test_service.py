...
def test_create_vlan_service_instance():
    ...
    reslist = helper.post_and_check('/service/vlan_service',pl)
    res = helper.check_many(reslist)
    assert "(config)#int vlan 99" in res[0]["changes"][2]
    assert "(config)#int vlan 99" in res[1]["changes"][2]
...
def test_retrieve_vlan_service():
    ...
    reslist = helper.post_and_check('/service/vlan_service',pl)
    res = helper.check_many(reslist)
    assert res[0]["show int vlan 99"][0]["interface"] == "Vlan99"
    assert res[1]["show int vlan 99"][0]["interface"] == "Vlan99"
...
def test_delete_vlan_service_legacy():
    ...
    reslist = helper.post_and_check('/service/vlan_service',pl)
    res = helper.check_many(reslist)
    assert "(config)#no int vlan 99" in res[0]["changes"][2]
    assert "(config)#no int vlan 99" in res[1]["changes"][2]
